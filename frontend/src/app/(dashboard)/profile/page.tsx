...
export default function ProfilePage() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-4 md:p-8 relative">
      {/* Accent Blur Orbs */}
      <div className="absolute top-0 right-1/4 w-[350px] h-[350px] bg-purple-500/5 rounded-full blur-[120px] pointer-events-none" />
      <div className="absolute bottom-0 left-1/4 w-[350px] h-[350px] bg-indigo-500/5 rounded-full blur-[120px] pointer-events-none" />
      ...
      <div className="max-w-5xl mx-auto relative z-10">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 pb-6 border-b border-slate-900">
          <div>
            <h1 className="text-3xl font-bold bg-gradient-to-r from-purple-400 via-indigo-200 to-indigo-400 bg-clip-text text-transparent">
              Account Settings
            </h1>
          ...
          <button
            onClick={logout}
            className="px-4 py-2.5 bg-slate-900 border border-slate-800 text-slate-300 hover:text-white hover:bg-slate-850 hover:border-red-500/30 rounded-lg text-sm transition duration-200 flex items-center gap-2"
          >
            <svg
              className="h-4 w-4"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <div className="bg-slate-900/30 border border-slate-900/80 rounded-2xl p-6 backdrop-blur-xl">
              <div className="flex flex-col items-center text-center">
                <div className="h-20 w-20 rounded-full bg-gradient-to-tr from-purple-600 to-indigo-600 flex items-center justify-center text-white text-2xl font-bold mb-4 shadow-lg ring-4 ring-purple-500/10">
                  {currentUser.name ? currentUser.name.charAt(0).toUpperCase() : 'U'}
                </div>
                <h2 className="text-xl font-bold text-white">{currentUser.name}</h2>
                <span className="px-2.5 py-0.5 mt-1.5 bg-purple-500/10 border border-purple-500/20 text-purple-400 text-xs font-semibold rounded-full uppercase tracking-wider">
                  {currentUser.role}
                </span>
                <p className="text-slate-400 text-sm mt-3 px-2 italic">
                  {currentUser.bio || '"No bio written yet."'}
                </p>
              </div>

              <div className="mt-6 space-y-3.5 border-t border-slate-900 pt-6 text-sm text-slate-300">
            ...
            <div className="bg-slate-900/30 border border-slate-900/80 rounded-2xl p-6 backdrop-blur-xl">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="p-4 bg-slate-950/40 border border-slate-900 rounded-xl">
                  ...
                  <span className="block text-2xl font-bold text-indigo-400 mt-1">0</span>
                  <span className="text-[10px] text-slate-500">total runs</span>
                </div>
              </div>
              <div className="mt-4 p-3 bg-purple-950/10 border border-purple-500/10 rounded-xl text-center text-xs text-purple-400">
              ...
              <form onSubmit={handleUpdateProfile} className="space-y-4">
                ...
                <div className="mb-4">
                  ...
                  <textarea
                    id="bio"
                    rows={3}
                    className="w-full px-3.5 py-2 bg-slate-900/60 border border-slate-800 focus:border-purple-500/60 focus:ring-purple-500/20 rounded-lg text-slate-100 placeholder-slate-500 focus:outline-none focus:ring-4 transition duration-200 backdrop-blur-sm"
